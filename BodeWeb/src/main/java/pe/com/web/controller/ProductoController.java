...
/**
 * @author Hans Soto Rojas
 * @email hans.sotor@gmail.com
 */
@ManagedBean
@SessionScoped
public class ProductoController implements Serializable {
    ...
    public void insertar() {
        try {
            ...
            WebUtil.mensajeInformacion(WebUtil.obtenerPropiedad("productoController.guardarExito", this.productoGuardar.getId()), WebUtil.obtenerPropiedad("productoController.guardarExito", this.productoGuardar.getId()));
            this.limpiar();
    ...
    public void actualizar() {
        try {
            ...
            WebUtil.mensajeInformacion(WebUtil.obtenerPropiedad("productoController.actualizarExito", this.productoGuardar.getId()), WebUtil.obtenerPropiedad("productoController.actualizarExito", this.productoGuardar.getId()));
        } catch (Exception exception) {
            String mensaje = WebUtil.controlarError(exception, LOGGER);
            WebUtil.mensajeError(mensaje, mensaje);
        }
    ...
    public void eliminar() {
        try {
            if (this.productoSeleccionado != null && this.productoSeleccionado.getNombre().length() >= 0) {
                ...
                WebUtil.mensajeInformacion(WebUtil.obtenerPropiedad("productoController.eliminarExito", this.productoSeleccionado.getId()), WebUtil.obtenerPropiedad("productoController.eliminarExito", this.productoSeleccionado.getId()));
                this.limpiar();
            } else {
